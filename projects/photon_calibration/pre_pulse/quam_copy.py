...
class Angle2volt_f12(object):
    ...
    def __eq__(self, other):
        if isinstance(other, Angle2volt_f12):
            return f"{self._path}{self._index}" == f"{other._path}{other._index}"
        return NotImplemented
...
class Drag_cosine(object):
    ...
    @property
    def angle2volt_f12(self) -> Angle2volt_f12:
        """Rotation angle (on the Bloch sphere) to voltage amplitude conversion for the f12 transition, must be within [-0.5, 0.5) V. For instance 'deg180':0.2 will lead to a pi pulse of 0.2 V."""
        return Angle2volt_f12(
            self._quam, self._path + "angle2volt_f12/", self._index,
            self._schema["properties"]["angle2volt_f12"]
        )
    @angle2volt_f12.setter
    def angle2volt_f12(self, value: dict):
        import quam_sdk.crud
        quam_sdk.crud.replace_layer_in_quam_data(self, "angle2volt_f12", value)
