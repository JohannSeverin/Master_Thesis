...
path = "/mnt/d/Data/master_thesis/trajectories/readout test_161128/dataset.nc"
...
# Open the data
data = xr.open_dataset(path, chunks={"sample": 100})
...
duration = 50000
for i in tqdm(range(data.I_ground.sample.size)):
    ax[0].plot(
        data.I_ground.isel({"sample": i})[:duration].cumsum("adc_timestamp"),
        data.Q_ground.isel({"sample": i})[:duration].cumsum("adc_timestamp"),
        color="C0",
        ls="-",
        alpha=0.10,
    )
    ax[0].plot(
        data.I_excited.isel({"sample": i})[:duration].cumsum("adc_timestamp"),
        data.Q_excited.isel({"sample": i})[:duration].cumsum("adc_timestamp"),
        color="C1",
        ls="-",
        alpha=0.10,
    )
...
ax[1].set(title="First 2µs Trajectories", xlabel="I", ylabel="Q")
duration = 200
for i in tqdm(range(data.I_ground.sample.size)):
    ax[1].plot(
        data.I_ground.isel({"sample": i})[:duration].cumsum("adc_timestamp"),
...
for i in tqdm(range(data.I_ground.sample.size)):
    ax[2].plot(
        (
            data.I_ground.isel({"sample": i}).cumsum("adc_timestamp")
            / (np.arange(len(times)) + 1)
        )[warm_up:duration],
        (
            data.Q_ground.isel({"sample": i}).cumsum("adc_timestamp")
            / (np.arange(len(times)) + 1)
        )[warm_up:duration],
        color="C0",
        ls="-",
    ...
    ax[2].plot(
        (
            data.I_excited.isel({"sample": i}).cumsum("adc_timestamp")
            / (np.arange(len(times)) + 1)
        )[warm_up:duration],
        (
            data.Q_ground.isel({"sample": i}).cumsum("adc_timestamp")
...
ax[2].plot(
    (
        data.I_ground.mean("sample").cumsum("adc_timestamp")
        / (np.arange(len(times)) + 1)
    )[warm_up:],
    (
        data.Q_ground.mean("sample").cumsum("adc_timestamp")
        / (np.arange(len(times)) + 1)
    )[warm_up:],
    color="C0",
    ls="-",
...
ax[2].plot(
    (
        data.I_excited.mean("sample").cumsum("adc_timestamp")
        / (np.arange(len(times)) + 1)
    )[warm_up:],
    (
        data.Q_excited.mean("sample").cumsum("adc_timestamp")
        / (np.arange(len(times)) + 1)
    )[warm_up:],
    color="C1",
    ls="-",
...
ax[3].set(title="Cumulative Meaned Trajectories First 2µs", xlabel="I", ylabel="Q")
duration = 200
warm_up = 10
for i in tqdm(range(data.I_ground.sample.size)):
    ax[3].plot(
    ...
    ax[3].plot(
        ...
        (
            data.Q_excited.isel({"sample": i}).cumsum("adc_timestamp")
            / (np.arange(len(times)) + 1)
        )[warm_up:duration],
        color="C1",
        ls="-",
